...
当前仓库推荐使用的实现是 `V1.7-532-4F` 自定义黑白传输主线：固定 `532x532` 逻辑网格、三个大角点 Finder + 一个小 Alignment、`16x3` header 保留区、2 像素安全边框。
...
- 默认输出尺寸：`5320x5320`（逻辑帧 532×532，放大 10 倍）
- 默认模块尺寸：`module_px = 10`
- 默认单帧载荷上限：`30114 bytes/frame`
- 推荐命令：`Project1 encode input.bin out/encode/input`

## 重要提醒

- 当前默认 CLI、README、样例和联调口径都以 `V1.7-532-4F` 为准。
