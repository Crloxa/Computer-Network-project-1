...
namespace ImgParse133
{
	...
	bool FindForthPoint(vector<QrcodeParse133::ParseInfo>& PointsInfo)
	{
		float avgSize = (PointsInfo[0].size + PointsInfo[1].size + PointsInfo[2].size) / 3.0;//以平均面积为基准寻找第四个点
		float expectDistance = distance(PointsInfo[0].Center, CalForthPoint(PointsInfo[0].Center, PointsInfo[1].Center, PointsInfo[2].Center));//以前三个点的平均面积为基准计算出第四个点与第一个点的期望距离
		QrcodeParse133::ParseInfo possiblePoints;//可能的第四个点
		bool tag = 1;//是否找到合法的第四个点
		for (int i = 3; i < PointsInfo.size(); ++i)//遍历剩余的点，寻找合法的第四个点
		{
	...
	vector<Point2f> AdjustForthPoint(const vector<QrcodeParse133::ParseInfo> PointsInfo, bool tag)
	{
		float avglen = 0.0;
		vector<Point2f> ret;
		int id[4][3] = { { 0,1,2 },{1,0,3},{2,0,3},{3,1,2} };
	...
	void GetVec(Mat& mat)
	{
		// 修改1：扩大采样区域，改为图像中心的合理区域
		int roiSize = min(200, mat.rows / 4);  // 取图像的1/4区域
		int start = mat.rows / 2 - roiSize / 2;
		int end = start + roiSize;
	...
	vector<Point2f> FindConner(Mat& mat)
	{
		...
		int poi[4][2] = { {0,0},{1329,0},{0,1329},{1329,1329} };
		vector<Point2f> ret;
		for (int k = 0; k < 4; ++k)
		{
			bool ispass[16][16] = { 0 };
	...
	bool Main(const Mat& srcImg, Mat& disImg)
	{
		...
		//以下为定位过程
		{
			vector<QrcodeParse133::ParseInfo> PointsInfo;
			...
			disImg.copyTo(temp);
			cv::resize(temp, temp, Size(1330, 1330));
			//blur(temp, temp, Size(5, 5));
			cv::medianBlur(temp, temp, 5);  // 5x5的中值滤波
			GetVec(temp);
			auto poi4 = FindConner(temp);
			if (poi4.size() != 4) return 1;
			cv::resize(disImg, disImg, Size(1330, 1330));
			temp = CropParallelRect(disImg, poi4, Size(1330, 1330));
		}

		disImg = temp;
		GetVec(disImg);
		Resize(disImg);
		resize(disImg, disImg, Size(133 * 3, 133 * 3));
		return 0;
	}
}