...
namespace ImgParse
{
    ...
    bool isRightlAngle(float angle, float tolerance)
    {
        // 验证输入角度的合法性
    ...
    //支持自定义比例范围
    bool IsQrTypeRateLegal(float rate, float minRate , float maxRate )
    {
    ...
    bool isLegalDistanceRate(float rate, float expectedDistance,
        float minRelativeTolerance , float maxRelativeTolerance )
    {
        // 根据距离大小自适应调整容差
